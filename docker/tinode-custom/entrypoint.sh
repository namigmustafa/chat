...
# CUSTOMIZED: upstream redirects stderr to /var/log/tinode.log here, which made
# any startup crash invisible to "az containerapp logs" / Log Analytics (only
# init-db's output was ever visible). Let stderr go to the container's normal
# stderr stream instead so real crash reasons show up in platform logs.
exec ./tinode "${args[@]}"
